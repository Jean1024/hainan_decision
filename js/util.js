!function(U){
	...
	/*根据定位得到城市信息*/
	U.getGeoInfo = function(successCallback,onerrorFn){
		var timeout = 30000;
	    ...
	    var fn_success = function(position){
            ...
            U.getJson(U.getEncodeUrl('http://geo.weather.com.cn/ag9/?lon='+lon+'&lat='+lat),function(json){
	...
	U.getAppInfo = function(callback,no_refresh){
		var appinfo = _localstorage.get('appinfo');
		function refresh(cb){
			if(no_refresh){
				return;
			}
			setTimeout(function(){
				var appinfo_url = DEFAULT_CONFIG.appinfo_url;
				if(appinfo_url){
					U.getJson(appinfo_url,function(data){
						loadedRealData = true;
						_localstorage.set('appinfo',data);
						cb && cb(data,3);
					});
				}
			},1000);
		...
		if(!appinfo){
			...
			callback(DEFAULT_CONFIG.appinfo,2);
			refresh();
		}else{
			callback(appinfo,1);
			refresh();
...
!function(nav){
	...
	nav.init = function(callback){
		...
		U.getAppInfo(function(appinfo){
			...
			$.each(list,function(i,v){
				...
				var sort_middle = v.small;
				var sub_html = '';
				var is_middle_sort_on = false;
				if(sort_middle){
					var str = '';
					var sid;//要跳过的分类ID
					$.each(v.small,function(s_i,s_v){
						var name = s_v.name;
						var s_id = s_v.id;
						...
						str += '<li class="'+(twoId == s_id && (is_middle_sort_on = true)?' on':'')+'" data-id="'+id+'" data-sid="'+s_id+'" data-style="'+c_style+'"'+dataUrl+'>'+name+'</li>';
					});
					sub_html = '<li class="s" '+(is_middle_sort_on?'style="display:block"':'')+'><ul>'+str+'</ul></li>';
				}
				html += '<li '+(columnId == id && !is_middle_sort_on?'class="on"':'')+' '+(sid?'data-sid="'+sid+'"':'')+' data-id="'+id+'" data-style="'+columnstyle+'"'+(v.module == 'weburl'?' data-url="'+v.dataurl+'"':'')+'>'+v.name+'</li>'+sub_html;
			});
			...
			var scroll_nav = Util.getScroll($nav.find('.nav_left_c').get(0),{
				scrollX: false
			});
			//滚动到当前选中的分类
	...
	!function(Top){
		...
		var trigger = function(data){
			...
			var is_geomap = (module == 'json' || module == 'imgs' && data.typeproperty == 1);
			...
			if(sort_item){
				...
				if(sort_item.type == TYPE_CONTENT || sort_item.columnstyle == COLUMNSTYLE_TYPHOON || (!list || list.length == 0)){
					...
					if(list && list.length > 0){
						...
						if(sort_item_middle){
							sort_item_small = sort_item_middle.small;
							...
							if(sort_item_small && sort_item_small.length > 0){
								...
								_returnData = {
									...
									on_id: 0,
									data: {
										l_url: sort_item_middle.listdataurl,
										d_url: sort_item_middle.dataurl,
										module: sort_item_middle.module,
										type: sort_item_middle.type,
										cstyle: sort_item_middle.columnstyle,
										typeproperty: sort_item_middle.typeproperty
									}
								};
							}
							var name = sort_item_middle.name;
							if(name == _specialSortName){
								name = sort_item.name;
							}
		...
		var _render = function(data,change_callback){
			...
			if(type == TYPE_SMALL_SORT){
				...
				$.each(list,function(i,v){
					var _id = v.id;
					html += '<li class="fl'+(on_id == _id?' on':'')+'" data-id="'+_id+'">'+v.name+'</li>';
				});
				html += '</ul></div>';
				var $nav_top_c = $(html);
