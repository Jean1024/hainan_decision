...
define("GeoMap", ["zrender", "zrender/tool/util"],
	function(zrender, util) {
		...
		function GeoMap(id, cfg) {
			...
			seft.makePoint = function(){
				return convertor.makePoint.apply(convertor,arguments);
			}
			...
			seft.render = function(callback) {
				seft.zr.render(callback);
			}
			...
			seft.loadWeather = function(json, showid, cfg) {
				...
				seft.refresh(function(){
					...
					seft.weatherpaths = json?convertor.json2path(json, seft):[];// 防止json为null
			...
			seft.refreshWeather = function(showid) {
				...
				seft.loadWeather(seft.weatherjson,showid);
			},
