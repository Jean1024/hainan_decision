...
function initMap(idName,data){
  ...
  $('.content.nav_animate').css('marginTop','50px')
  // 添加右下方标识
...
!function(U){
	...
	U.getEncodeUrl = function(url){
      ...
      var private_key = 'chinaweather_jcb_webapi_data';
	    // var private_key = 'open_data';
	    var key = base64encode(str_hmac_sha1(private_key, url));
	    key = encodeURIComponent(key);
      console.log(url.replace(/appid=.*/,'appid='+appid.substr(0,6)) + '&key=' + key)
...
!function() {
  ...
  MinuteLevelRain.prototype={
    ...
    onLoad :function () {
      ...
      this.map.centerAndZoom(new T.LngLat( 105, 38 ), this.zoom);
